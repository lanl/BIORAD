...
class Tensor : public std::vector<T>
{
    ...
    public:
        ...
        inline T& operator()(const std::vector<size_type> &m_index)
        {
            #ifdef EBUG
            if( length.size() != m_index.size() ){
                throw __FILE__ ":Tensor(vector index): Index has incorrect dimensionality";
            }

            if(length.empty()){
                throw __FILE__ ":Tensor(vector index): Cannot index an empty tensor";
            }
            #endif // EBUG

            size_type master_index = 0;
            ...
            for(size_t i = 0;i < d;++i){
                ...
                #ifdef EBUG
                if(m_index[i] >= length[i]){
                    throw __FILE__ ":Tensor(vector index): Index is out of bounds";
                }
                #endif // EBUG
                
                master_index = master_index*length[i] + m_index[i];
            }

            #ifdef EBUG
            if( master_index >= std::vector<T>::size() ){
                throw __FILE__ ":Tensor(vector index): Master index is out of bounds";
            }
            #endif // EBUG
        ...
        inline const T& operator()(const std::vector<size_type> &m_index) const
        {
            #ifdef EBUG
            if( length.size() != m_index.size() ){
                throw __FILE__ ":Tensor(vector index): Index has incorrect dimensionality";
        ...
        void clamp(const T& m_max_value)
        {
            for(typename std::vector<T>::iterator i = std::vector<T>::begin();i != std::vector<T>::end();++i){
                *i = std::min(*i, m_max_value);
            }
        };
